# This Makefile is for the DMI::Decode extension to perl.
#
...
#     VERSION_FROM => q[Decode.pm]
#     clean => { FILES=>q[_Inline Decode.inl] }
...
# Where to put things:
INST_LIBDIR      = $(INST_LIB)/DMI
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/DMI
...
TO_INST_PM = Decode.pm

PM_TO_BLIB = Decode.pm \
	blib/lib/DMI/Decode.pm
...
manifypods : pure_all Decode.pm
	@$(POD2MAN) \
	Decode.pm \
	$(INST_MAN3DIR)/DMI::Decode.$(MAN3EXT)
...
clean ::
	-rm -rf _Inline Decode.inl ./blib $(MAKE_APERL_FILE) $(INST_ARCHAUTODIR)/extralibs.all perlmain.c tmon.out mon.out so_locations pm_to_blib *$(OBJ_EXT) *$(LIB_EXT) perl.exe perl perl$(EXE_EXT) $(BOOTSTRAP) $(BASEEXT).bso $(BASEEXT).def lib$(BASEEXT).def $(BASEEXT).exp $(BASEEXT).x core core.*perl.*.? *perl.core
...
ppd:
	@$(PERL) -e "print qq{<SOFTPKG NAME=\"$(DISTNAME)\" VERSION=\"2,04,0,0\">\n\t<TITLE>$(DISTNAME)</TITLE>\n\t<ABSTRACT></ABSTRACT>\n\t<AUTHOR></AUTHOR>\n}" > $(DISTNAME).ppd
	@$(PERL) -e "print qq{\t<IMPLEMENTATION>\n\t\t<DEPENDENCY NAME=\"Inline\" VERSION=\"0,44,0,0\" />\n}" >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	...
	-e "pm_to_blib({qw{Decode.pm blib/lib/DMI/Decode.pm}},'$(INST_LIB)/auto','$(PM_FILTER)')"
	@$(TOUCH) $@
...
.pm.inl:
	$(PERL) -Mblib -MInline=NOISY,_INSTALL_ -MDMI::Decode -e1 2.04 $(INST_ARCHLIB)
